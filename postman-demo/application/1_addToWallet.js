...
async function main(certificatePath, privateKeyPath, iL, id, org ) {

	const wallet = new FileSystemWallet(org);
	...
	try {
		...
		const identityLabel = iL;
		// const identity = X509WalletMixin.createIdentity('manufacturerMSP', certificate, privatekey);
		const identity = X509WalletMixin.createIdentity(id, certificate, privatekey);
